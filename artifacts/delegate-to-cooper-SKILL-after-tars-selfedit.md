...
# no_active_sender_terminal. -> the run id (expected at result.run.id — this
# field name is NOT confirmed live). Read the JSON myself; if that path is
# absent, take whatever id the response does carry and report the real shape.
...
#   Current Orca requires --terminal over plain SSH; omitting it returns
#   no_active_terminal even when --run is supplied. Expect ok:true with runId echoed back (count 0 is fine — nothing has been
...
#   Inspect the returned agent terminal preview; when the prompt is sitting at
#   the input line, submit it explicitly:
#     orca terminal send --terminal "<AGENT_TERMINAL_HANDLE>" --enter --json
#   Then verify activity with worker-show/worktree ps before beginning the wait.
