...
| IDs: Gaetan `U08BDJAMSRZ` · Tars bot user `U0BBH85NAKH` · app `A0BC0GXH78R` · team `T7V1UGJ82` · home DM `D0BBYNM01BL` — **sole interaction + delivery surface since 2026-08-13** · reporting channel `C0BP2GZUFSR` = `#gcn-tars-reporting`, private (created 2026-08-10) — **RETIRED 2026-08-13**: all interaction and deliveries moved to the DM · test channel `C08RWSTU9LK`. **`C0BFQ5WFYTB` = `#tech-project-support-engineer` is a live TEAM channel — never post there** (an operator directive once named it as the test channel; it was inverted). VM env names: `SLACK_MCP_XOXC_TOKEN` / `SLACK_MCP_XOXD_TOKEN` (cookie sent as-stored, URL-encoded) | `wf4/p01`, `wf6/slack-channel-audit` |
| `slack_read_channel` (claude.ai Slack connector) returns bot messages **by default** — there is no `include_bots` parameter. Measured live 2026-08-13; supersedes the earlier claim below that one was required | measured 2026-08-13 |
| **RETIRED 2026-08-13**: the reconciler cron job is disabled, and the three report jobs' `deliver` now points flat at the home DM `slack:D0BBYNM01BL` — no parent, no threading, no reconciliation. Kept below for archaeology: the three report jobs' `deliver` (`e231e5faf180`, `62e8cd9db637`, `759e08c598e3`) **was** reconciler-managed — never hand-edit it. `scripts/tars-report-thread.py` (no-agent `--script` cron job, `25,55 7-16 * * 1-5` Paris) rewrote it each tick to `slack:C0BP2GZUFSR:<today's parent ts>`, or bare `slack:C0BP2GZUFSR` until the day's first top-level bot message existed. It read `conversations.history` and posted nothing itself; a hand-set static ts was exactly the stale-thread bug it fixed | `daily-report-threading` spec |
...
| **Capability and narration are separate axes (2026-08-10).** Toolsets are identical in every validly-invoked Slack conversation — the `platform_tool_conversations` capability gate is withdrawn, never re-introduce it. Interim tool narration is chat-ID-exact: `display.platforms.slack.tool_progress_conversations: {C0BP2GZUFSR: all, D0BBYNM01BL: all}` over `tool_progress: "off"` (quoted — bare `off` is YAML-1.1 `False`); `tool_progress_dm` is deliberately unset — the adapter collapses MPIMs into chat_type `dm` (`is_dm = channel_type in {"im","mpim"}`), so the chat-type override would narrate group DMs. **The `C0BP2GZUFSR` entry is stale since the channel's 2026-08-13 retirement** — `D0BBYNM01BL` is now the only live conversation this map needs to cover. **SUPERSEDED 2026-08-13: both patches dropped, checkout is vanilla upstream `6e87d43`** — neither `tool_progress_conversations` nor `tool_progress_dm` exists any more; narration is governed solely by `display.platforms.slack.tool_progress: "off"`, which upstream applies uniformly to every chat type (no chat-type branch exists in the un-patched resolver). The capability/narration axis separation itself still holds | `wf5/slack-tool-visibility-correction`, `wf6/slack-tool-visibility-restore`, `probes/patch-drop-2026-08-13` |
