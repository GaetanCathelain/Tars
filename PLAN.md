...
| WF4 verify | 14 probes ∥ + critic, barrier, report | Every capability exercised once with evidence before Tars is declared live ("done is not exercised"). Includes the negative test: a message from someone who isn't Gaetan must be ignored. Gaps found by the critic loop back as new probes. **Probe 15 added 2026-08-07 (Gaetan):** Tars-initiated delegation-lite to cooper — one chat turn has Tars run a read-only command over the mesh (`ssh cooper hostname`) and report the output, proving the model-loop→shell-tool→ssh chain, not just transport. |
| WF5 behaviors | triaged 2026-08-07 (Gaetan) | **NOW (workflows dispatched):** Orca delegation playbook (Tars→cooper Orca sessions, Slack E2E tests allowed) · Kanban (Hermes ships gated kanban tooling — `_check_kanban_mode` currently False). **LATER:** behavior-layer spec (dailys, reminders, reports) · A2A investigation (below) · GBrain/mc-kestra data · Hindsight memory. **CANCELLED:** Notion exports — MCP live-fetching only. **A2A investigation (added 2026-08-07, Gaetan, → later):** check real agent-to-agent integration Tars ↔ cooper and Tars ↔ p-Hermes. Today: Tars serves an inbound agent card on `127.0.0.1:9900` only (probe 12), outbound toolset disabled (D2 YAGNI), p-Hermes A2A state unknown, cooper runs no Hermes A2A endpoint (Orca sessions speak SendMessage/orc-tab, a different protocol). Scope: inventory both endpoints, pick a transport (SSH-tunneled localhost vs tailnet bind vs keep-disabled), define guardrails (what a remote agent may ask Tars; what Tars may ask p-Hermes), then probe a real round-trip each way. |
