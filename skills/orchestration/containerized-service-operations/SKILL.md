...
- Do not infer the launch command or working directory from the process name, image default, container `pwd`, or entrypoint alone. Read image/container metadata and the executable's own help or package metadata. For a relative executable path, reconstruct both the listener's NUL-delimited `/proc/<pid>/cmdline` and `/proc/<pid>/cwd`; before stopping it, verify inside the container that the recorded working directory exists and that the relative executable resolves there. Relaunch with `docker exec -w <recorded-cwd>` and the exact recorded argv so it uses the same checkout. A keeper container's default working directory can differ from a detached service's cwd; substituting that default can make detached `docker exec` exit silently and leave the port down. Immediately before signaling, revalidate that the recorded PID still owns the port and still belongs to the recorded container.
- For a secret-safe authenticated proxy probe, an existing API key may be read from the listener's NUL-delimited `/proc/<pid>/environ` inside a privileged on-host script and kept only in memory. Never print it, put it in shell argv, or copy it off-host. Use it for one minimal real completion/message request and report only status plus a deliberately non-secret response fact; a successful models/catalog request is not enough. When model names are dynamic, query the authenticated model catalog in memory, choose an ID it actually returns, then issue the real completion/message request; do not guess a stale model alias.
- Treat Tailscale Serve edits as compare-and-swap operations: snapshot the complete `tailscale serve status --json`, revalidate the exact host/port/path/backend immediately before mutation, use the narrow listener-specific `tailscale serve --bg --https=<port> <backend>` command rather than `reset` or whole-config replacement, then normalize only the intended field and compare the complete before/after JSON. Abort rather than broadly overwrite if the target moved concurrently or any non-target entry differs. See `references/tailscale-serve-surgical-route-update.md`.
...
- For deploying a second version on a new port inside the same keeper container without interrupting the existing detached service, read `references/side-by-side-process-deploy.md`.
...
- For narrow compare-and-swap Tailscale Serve route changes with full non-target preservation and secret-safe end-to-end proof, read `references/tailscale-serve-surgical-route-update.md`.
