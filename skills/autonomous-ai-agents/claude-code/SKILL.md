...
## Orca / Cooper standing requirements

All agent work on Orca OR Cooper must use at least 1M effective context, across
all models and work types, including launches, resumes and recoveries. This
also covers direct CLI work on Cooper. Preserve explicit Claude Opus 4.8 pins
where applicable; a context requirement does not authorize a model change.
Explicitly select a supported long-context configuration, then verify both the
effective model and actual context capacity from that session/runtime (such as
`modelUsage.contextWindow` or `/context`). Documented `[1m]` selector syntax is
not a guarantee for an unverified full-model selector/account/runtime. Verify
before task execution and again on resume/recovery; a smaller or unknown
capacity is a blocker, never a silent fallback. Keep the same session ID when
recovering; do not fork. Generic model/fallback examples below do not override
these requirements. Hermes own operating records are edited by Hermes itself,
never delegated to Claude/Orca (SOUL rule 2).
...
❯ 2. No, exit                    ← observed on Cooper (2026-09-05)
```
**Handling:** Capture the actual screen first; do not assume the default is Yes.
For the named, known-owned authorized workspace only, explicitly select the
trust choice shown (Up then Enter for the screen above), then read back the
screen. Enter alone on the observed No/exit selection exits Claude. Do not
blindly trust an unknown folder or send repeated Enter.
...
# Read the actual dialog, then explicitly select trust only for the authorized folder.
terminal(command="tmux capture-pane -t claude-work -p -S -50")
# Only if the screen shows option 2 No/exit selected and option 1 is the trust choice:
terminal(command="tmux send-keys -t claude-work Up Enter && tmux capture-pane -t claude-work -p -S -50")
...
terminal(command="tmux capture-pane -t review -p -S -50")  # Inspect trust dialog; explicitly select the authorized folder's trust choice, never blind Enter.
