...
3. Bind the send to the authorized requester, exact recipient/conversation and thread (or explicitly no thread), sending identity, payload and exposure boundary. For Tars, use the gateway identity, never the personal Slack MCP sender. Confirm destination membership and SOUL's per-message authority; initiated notifications go to a new top-level home DM message. A rejected/failed send never authorizes changing target, identity or transport policy. Preserve the exact requested payload—especially “only this GIF.” A private URL request does not authorize publication.
